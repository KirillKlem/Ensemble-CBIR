# Проект Content-Based Image Retrieval (CBIR)
...
Для оценки работы проекта необходимо сгенерировать файл `submission.csv` по следующим требованиям:

- Два столбца: `image` и `recs`.
- Столбец `recs` должен содержать 10 названий рекомендованных изображений (без расширения и разделённых запятыми, заключённых в кавычки).
- Сам файл разделяется запятой.
